        ...
        {selectedLayer ? (
          <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-3 py-1.5 rounded-2xl transition-all duration-200">
            ...
            <input
              ...
              className="w-36 rounded-xl border border-white/10 bg-white/10 px-2.5 py-1 text-xs text-white outline-none placeholder:text-white/30 focus:border-violet-500/50"
            ...
            <select
              ...
              className="rounded-xl border border-white/10 bg-[#1f2937] px-2 py-1 text-xs text-white outline-none cursor-pointer hover:bg-white/15 max-w-[100px]"
            >
            ...
            {/* Font Size Input */}
            <div className="flex items-center rounded-xl border border-white/10 bg-white/10 px-1 py-0.5 text-white">
              <button 
                onClick={() => updateLayer(selectedLayer.id, { fontSize: Math.max(10, selectedLayer.fontSize - 2) })}
                className="px-1.5 py-0.5 text-xs hover:bg-white/10 rounded-lg cursor-pointer"
              >
                -
              </button>
              <input
                ...
                onChange={(e) => updateLayer(selectedLayer.id, { fontSize: Math.max(10, Number(e.target.value)) })}
                className="w-8 text-center bg-transparent text-xs outline-none border-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              />
              <button 
                onClick={() => updateLayer(selectedLayer.id, { fontSize: selectedLayer.fontSize + 2 })}
                className="px-1.5 py-0.5 text-xs hover:bg-white/10 rounded-lg cursor-pointer"
              >
                +
              </button>
            ...
            {/* Color Input */}
            <div className="flex items-center gap-1 rounded-xl border border-white/10 bg-white/10 px-1.5 py-0.5 text-white">
              <input
                type="color"
            ...
            <button
              ...
              className={`p-1.5 rounded-xl border border-white/10 hover:bg-white/15 transition cursor-pointer ${
            ...
            <button
              ...
              className="p-1.5 rounded-xl border border-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 transition cursor-pointer"
          ...
          <div className="text-xs text-white/35 font-medium italic">
            Select a text layer on the canvas to customize its typography and colors
          </div>
        )}
      </div>
